...
# And then: awk -f tables_without_pk_and_with_non_unique_key.awk db_schema.sql

BEGIN {
        is_create_table_start=0;
        has_pk=0;
        has_uk=0;
        has_non_unique_key=0;
        table_name="none";
        db_name="none";
}
...
/CREATE TABLE/ {
        if (table_name != "none" && has_pk == 0 && has_uk == 0 && has_non_unique_key == 1) {
...
/KEY/ {
        if (index($0, "UNIQUE") == 0 && index($0, "PRIMARY") == 0) {
                has_non_unique_key=1;
        }
}
